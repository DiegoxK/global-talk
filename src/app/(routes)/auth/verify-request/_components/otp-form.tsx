...
export function InputOTPForm({ email }: { email: string }) {
  ...
  function onSubmit(data: z.infer<typeof FormSchema>) {
    ...
    const callbackUrl = encodeURIComponent(`${getBaseUrl()}/academy`);
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-2 px-4">
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl className="justify-center">
                ...
                >
                  <InputOTPGroup className="rounded-md border-2 border-primary bg-white uppercase">
        ...
        <Button
          disabled={loading}
          className="w-full rounded-md bg-primary-800"
          type="submit"
        >
          {loading ? <div className="loader"></div> : <span>Verificar</span>}
        </Button>
