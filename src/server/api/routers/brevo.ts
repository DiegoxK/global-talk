...
export const brevoRouter = createTRPCRouter({
  ...
  subscribe: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (!response.ok) {
        throw new Error(`Error al agregar el suscriptor a Brevo: ${response.statusText}`);
      }
  ...
  .mutation(async ({ input }) => {
    ...
    try {
      ...
      throw new Error("Hubo un error al eliminar el suscriptor. Por favor, inténtalo de nuevo.");
    }
  }),})